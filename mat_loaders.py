...
def get_data(dataset_name, datadir='../dataset', verbose=False, get_mtf=False):
    Yim = None
    eval_bands = [2,6]
    ...
    if dataset_name == 'apex':
        ...
        Yim = load_matlab_cell_array(os.path.join(datadir, 'avirisLowCrops.mat'), 'Yim', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'avirisLowCrops.mat'), 'Xm_im', verbose)
    elif dataset_name == 'coast':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'avirisLowCoast.mat'), 'Yim', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'avirisLowCoast.mat'), 'Xm_im', verbose)
    elif dataset_name == 'escondido':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'escondido.mat'), 'Yim', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'escondido.mat'), 'Xm_im', verbose)
    elif dataset_name == 'escondido_s2':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'escondido.mat'), 'S2', verbose)
        Xm_im = np.zeros((Yim[1].shape[0], Yim[1].shape[1], len(Yim)))*np.nan
        eval_bands = None
    elif dataset_name == 'mountain':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'avirisLowMontain.mat'), 'Yim', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'avirisLowMontain.mat'), 'Xm_im', verbose)
    elif dataset_name == 'rkvik':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'rkvik_crop.mat'), 'Yim', verbose)
        Xm_im = np.zeros((Yim[1].shape[0], Yim[1].shape[1], len(Yim)))*np.nan
        eval_bands = None
    elif dataset_name == 'rkvik_rr_2':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'rkvik_crop.mat'), 'Yim_rr_2', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'rkvik_crop.mat'), 'Xm_im', verbose)
        eval_bands = [2]
    elif dataset_name == 'rkvik_rr_6':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'rkvik_crop.mat'), 'Yim_rr_6', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'rkvik_crop.mat'), 'Xm_im', verbose)
        eval_bands = [6]
    elif dataset_name == 'urban':
        Yim = load_matlab_cell_array(os.path.join(datadir, 'avirisLowCity.mat'), 'Yim', verbose)
        Xm_im = load_matlab_array(os.path.join(datadir, 'avirisLowCity.mat'), 'Xm_im', verbose)
