...
#
# Interface:
#           SSIM, SRE, RMSE, ERGAS, SAM = evaluate_performance(Xm_im,...
#                                                              Xhat_im,...
#                                                              limsub,...
#                                                              data_range,...
#                                                              d, bands)
...
def evaluate_performance(Xm_im, Xhat_im, limsub=0, data_range=1.0, d=[6, 1, 1, 1, 2, 2, 2, 1, 2, 6, 2, 2], bands=[2,6]):
    ...
    if average:
        ...
        bands_names = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'B11', 'B12']
        bands_names = [bands_names[i] for i in idx]
        ...
        SSIM_array, SSIM['All'] = ssim(X, Xhat, data_range=data_range)
        SSIM['Bands'] = {band: value for band, value in zip(bands_names, SSIM_array[idx])}

        SRE_array = sre(X, Xhat)
        SRE['Bands'] = {band: value for band, value in zip(bands_names, SRE_array[idx])}

        UIQI_array = uiqi(X, Xhat)
        UIQI['Bands'] = {band: value for band, value in zip(bands_names, UIQI_array[idx])}
        ...
        if 2 in bands:
            ...
            SSIM['20m'] = SSIM_array[idx2].mean()
            SRE['20m'] = SRE_array[idx2].mean()
            UIQI['20m'] = UIQI_array[idx2].mean()

        if 6 in bands:
            idx6 = np.squeeze(np.where(d == 6))
            ERGAS['60m'] = ergas(X[:,:,idx6], Xhat[:,:,idx6],6)
            RMSE['60m'] = rmse(X[:,:,idx6], Xhat[:,:,idx6])
            SSIM['60m'] = SSIM_array[idx6].mean()
            SRE['60m'] = SRE_array[idx6].mean()
            UIQI['60m'] = UIQI_array[idx6].mean()

        if 6 in bands and 2 in bands:
            RMSE['All'] = rmse(X[:,:,idx], Xhat[:,:,idx])
            SAM['All'] = sam(X[:,:,idx], Xhat[:,:,idx])
            SRE['All'] = SRE_array[idx].mean()
            UIQI['All'] = UIQI_array[idx].mean()
